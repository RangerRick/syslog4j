 ...
 * @version $Id: StructuredSyslogMessage.java,v 1.2 2009/07/25 18:42:47 cvs Exp $
 */
public class StructuredSyslogMessage extends AbstractSyslogMessage implements StructuredSyslogMessageIF {
	private static final long serialVersionUID = 6414989888008909476L;
	 ...
	 * @param message
	 */
	public StructuredSyslogMessage(final String messageId, final Map<String, Map<String, String>> structuredData, final String message) {
		super();
	 ...
	 * @return
	 */
	public static StructuredSyslogMessage fromString(final String syslogMessageStr) {
	...
	private void deserialize(final String stringMessage) {
		...
		if (!SyslogConstants.STRUCTURED_DATA_EMPTY_VALUE.equals(structuredDataString)) {
			while (!"".equals(structuredDataString)) {
				if (!structuredDataString.startsWith("[") || structuredDataString.indexOf(']') == -1) {
	...
	private String serialize() {
		...
		if (getStructuredData() == null || getStructuredData().size() == 0) {
			...
			for (final Map.Entry<String, Map<String, String>> sdElement : getStructuredData().entrySet()) {
				...
				if (sdParams != null) {
					for (final Map.Entry<String, String> entry : sdParams.entrySet()) {
						...
						if (paramValue == null || paramValue.length() == 0)
							throw new IllegalArgumentException("Illegal structured data parameter value: " + paramValue);
	...
	public int hashCode() {
		...
		result = prime * result + ((structuredData == null) ? 0 : structuredData.hashCode());
		return result;
	}
	...
	public boolean equals(final Object obj) {
		...
		final StructuredSyslogMessage other = (StructuredSyslogMessage) obj;
		if (message == null) {
