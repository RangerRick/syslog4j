...
public class PrintStreamSyslogServerEventHandler implements SyslogServerEventHandlerIF {
	private static final long serialVersionUID = 5584736243576772245L;

	protected PrintStream stream = null;

	public PrintStreamSyslogServerEventHandler(final PrintStream stream) {
		this.stream = stream;
	}

	public void event(final SyslogServerIF syslogServer, final SyslogServerEventIF event) {
		final String date = (event.getDate() == null ? new Date() : event.getDate()).toString();
		final String facility = SyslogUtility.getFacilityString(event.getFacility());
